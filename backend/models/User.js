...
class User {
    // Emails are matched/stored case-insensitively (normalized to lowercase)
    // everywhere a user is looked up or created. Without this, a password
    // account registered as "Jane@Example.com" and a later "Continue with
    // Google" sign-in for "jane@example.com" (Google always lowercases the
    // email it returns) looked like two different people to a case-sensitive
    // SQL `=` / UNIQUE constraint - creating a second, empty account and
    // making the user's existing properties/tenants/payments appear to have
    // vanished when they signed in the other way. Normalizing here is the
    // single choke point that fixes it for every login method at once.
    ...
    static async findByEmail(email) {
        try {
            return await get('SELECT * FROM users WHERE email = ?', [this._normalizeEmail(email)]);
        } catch (error) {
        ...
        try {
            ...
            await run(
                `INSERT INTO users (id, name, email, password, google_id, is_verified, profile_pic)
                 VALUES (?, ?, ?, ?, ?, ?, ?)`,
                [id, name, this._normalizeEmail(email), hashedPassword, googleId || null, isVerified ? 1 : 0, profilePic || null]
            );
