// js/tenants.js - Complete Version with Loading Indicators
...
const Tenants = {
    ...
    async renderTable() {
                        ...
                        ${latestPayment 
                            ? (latestPayment.status === 'unpaid' 
                                ...
                                : latestPayment.status === 'partial'
                                    ...
                                    : `<span class="badge badge-success">Paid</span> <strong>${formatCurrency(totalWithoutDues)}</strong>`)
        ...
        document.addEventListener('scroll', closeDropdowns, { passive: true });
        document.addEventListener('touchmove', closeDropdowns, { passive: true });
    },
                ...
                " onmouseover="this.style.background='var(--bg-hover)'" onmouseout="this.style.background=''">
                    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 6px;">
                        ...
                        <div>
                            ...
                            <span style="margin-left: 8px; color: var(--text-light); display: inline-flex; vertical-align: middle;"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg></span>
                        </div>
                    </div>
    ...
    showDetails(id) {
        ...
        const property = App.state.properties.find(p => p.id === tenant.property_id);
        const roomInfo = property ? `<span class="property-name">${escapeHTML(property.name)}</span><span class="room-number">Room ${escapeHTML(String(tenant.room_number))}</span>` : 'Not assigned';
    ...
    showContactOptions(tenantId) {
        ...
        overlay.id = 'contactOptionsOverlay';
            ...
            <form id="tenantForm" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 16px;">
                <div style="display: flex; gap: 20px; align-items: flex-start;">
                    ...
                    <div style="flex: 1; display: flex; flex-direction: column; gap: 12px;">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                            <div class="settings-form-group">
                                <label style="font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.04em; color: var(--text-light); font-weight: 500; display: block; margin-bottom: 4px;">Mobile Number</label>
                        ...
                        <div class="settings-form-group">
                            <label style="font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.04em; color: var(--text-light); font-weight: 500; display: block; margin-bottom: 4px;">Lease End Date</label>
                            <input type="date" class="form-control" id="tenantLeaseEndDate">
                            <small style="color: var(--text-light); font-size: 0.65rem; display: block; margin-top: 2px;">Optional - powers the "lease ending soon" notification</small>
                        </div>
                    </div>
                </div>
    ...
    async saveTenant() {
        ...
        const mobileNumberInput = document.getElementById('tenantMobile');
        const advancePaymentInput = document.getElementById('tenantAdvancePayment');
        const mobileNumber = mobileNumberInput ? mobileNumberInput.value.trim() : '';
        const advancePayment = advancePaymentInput && advancePaymentInput.value !== '' ? parseFloat(advancePaymentInput.value) : 0;
        const leaseEndDateInput = document.getElementById('tenantLeaseEndDate');
        ...
        if (advancePayment < 0 || isNaN(advancePayment)) {
        ...
        if (profilePicFile) {
            if (!validateFileSize(profilePicFile, 5)) {
                Components.showError('Profile picture is too large. Please choose an image up to 5MB.');
                return;
            }
            if (!validateStrictImageType(profilePicFile)) {
                Components.showError('Unsupported file format. Profile pictures must be JPG, JPEG, PNG, or GIF.');
                return;
            }
        }
        
        if (documentFiles.length > 5) {
            Components.showError('You can upload a maximum of 5 documents per tenant.');
            return;
        }
        ...
        Components.showLoading('Adding tenant...');
        ...
        try {
            ...
            if (profilePicFile && profilePicFile.size > 0) {
                try {
                    ...
                    console.error('Profile picture read error:', fileError);
                    showNotification('Failed to read profile picture. Please try another image.', 'error');
            ...
            for (let i = 0; i < Math.min(documentFiles.length, 5); i++) {
                ...
                try {
                    ...
                    showNotification(`Failed to read document "${file.name}". Please try again.`, 'error');
                    Components.hideLoading();
                    this._isProcessing = false;
                    if (submitBtn) {
                        submitBtn.disabled = false;
    ...
    previewProfilePic(event) {
        ...
        if (!validateStrictImageType(file)) {
            Components.showError('Unsupported file format. Profile pictures must be JPG, JPEG, PNG, or GIF.');
            event.target.value = '';
            return;
        }
        ...
        reader.onload = function(e) {
            ...
            if (img && placeholder) {
                ...
                // Some formats (e.g. RAW camera files like CR2/NEF/ARW/DNG)
                // are accepted for storage but can't be rendered as an
                // <img> preview by the browser. Fall back to showing the
                // file extension instead of a broken-image icon.
    ...
    async editTenant(id) {
        ...
        if ((!properties || properties.length === 0) && !isDemoMode()) {
            ...
            <form id="tenantForm" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 16px;">
                ...
                <div style="display: flex; gap: 20px; align-items: flex-start;">
                    ...
                    <div style="flex: 1; display: flex; flex-direction: column; gap: 12px;">
                        ...
                        <div class="settings-form-group">
                            <label style="font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.04em; color: var(--text-light); font-weight: 500; display: block; margin-bottom: 4px;">Lease End Date</label>
                            <input type="date" class="form-control" id="tenantLeaseEndDate" value="${tenant.lease_end_date ? tenant.lease_end_date.slice(0, 10) : ''}">
                            <small style="color: var(--text-light); font-size: 0.65rem; display: block; margin-top: 2px;">Optional - powers the "lease ending soon" notification</small>
                        </div>
                        </div>
                    </div>
        ...
        setTimeout(async () => {
            ...
            propertySelect.addEventListener('change', async function() {
                ...
                if (isDemoMode()) {
                    ...
                    if (property && property.total_rooms) {
                        for (let i = 1; i <= property.total_rooms; i++) {
                            const isOccupied = App.state.tenants.some(t => t.property_id === propertyId && t.room_number === i && t.id !== currentTenantId);
                            if (!isOccupied) {
                                const selected = i === currentTenant?.room_number ? 'selected' : '';
                                roomSelect.innerHTML += `<option value="${i}" ${selected}>Room ${i}</option>`;
                            }
                        }
    ...
    async updateTenant() {
        ...
        Components.showLoading('Updating tenant...');
    ...
    async deleteTenant(id) {
        ...
        Components.showConfirm(
            ...
            async () => {
                this._isProcessing = true;
                Components.showLoading('Deleting tenant...');
