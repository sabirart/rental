...
async function handleGoogleLogin() {
    ...
    // Register buttons
    document.querySelectorAll('#registerTrigger, #heroRegister, #aboutRegister, #ctaRegister').forEach(el => {
        el?.addEventListener('click', function(e) {
            e.preventDefault();
            if (Auth.isAuthenticated) {
                window.location.href = 'dashboard.html';
    ...
    document.getElementById('heroRegister')?.addEventListener('click', function(e) {
        ...
        if (confirm(
            '⚠️ DEMO MODE\n\n' +
            'You are about to try the dashboard in demo mode.\n' +
            '• Data is stored temporarily in your browser\n' +
            '• All data will be lost when you refresh or close the page\n' +
            '• Create a free account to save your data permanently\n\n' +
            'Click OK to continue in demo mode, or Cancel to sign up.'
        )) {
            Auth.enableDemoMode();
            window.location.href = 'dashboard.html';
        } else {
